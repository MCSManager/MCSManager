...
function killPort(port) { try { execSync(`lsof -ti tcp:${port} 2>/dev/null | xargs kill -9 2>/dev/null || true`, { stdio: "ignore" }); } catch {} }
function killPattern(pat) { try { execSync(`pkill -9 -f ${JSON.stringify(pat)} 2>/dev/null || true`, { stdio: "ignore" }); } catch {} }
let procServer, procDaemon, procPanel;
function killChild(p) { try { if (p && !p.killed) p.kill("SIGKILL"); } catch {} }
function cleanup() {
...
function zipit(zipPath, entries, cwd) {
  ...
  execSync(`zip -rq ${JSON.stringify(zipPath)} ${entries.map((e) => JSON.stringify(e)).join(" ")}`, { cwd });
}
function buildPackages() {
  ...
  let slipSupported = true;
  {
    ...
    try {
      ...
      execSync(`zip -rq ${JSON.stringify(path.join(outDir, "malicious.zip"))} ../escape.txt app.js`, { cwd: d });
    } catch (e) {
      slipSupported = false;
    }
    ...
    if (slipSupported) {
      try {
        const listing = execSync(`unzip -l ${JSON.stringify(path.join(outDir, "malicious.zip"))}`, { encoding: "utf-8" });
...
async function main() {
  ...
  log("step 0: kill stale + reset data + restore baseline");
  [9999, 23333, 24444].forEach(killPort);
  ...
  let daemonUuid;
  {
    ...
    assert(ok, "panel connected to daemon");
    const r = await httpReq("GET", "/api/service/remote_services_list", undefined, token);
