...
//logo输出
// const LOGO_FILE_PATH = __dirname + "/core/logo.txt";
// 路径 Fix
const LOGO_FILE_PATH = path.join(__dirname, "core", "logo.txt");
...
//自动装载所有路由
let routesPath = path.join(__dirname, "route");
// console.log(routesPath);
let routeList = fs.readdirSync(routesPath);
...
(function initializationRun() {
	...
	try {
		...
		if (!fs.existsSync(SERVER_CORE_PATH)) fs.mkdirSync(SERVER_CORE_PATH);
		if (!fs.existsSync(SERVER_SCH_PATH)) fs.mkdirSync(SERVER_SCH_PATH);
		...
		if (!fs.existsSync(CENTEN_LOG_JSON_PATH))
			tools.mCopyFileSync(
				// 多用join
				path.join(INIT_CONFIG_PATH, "info_reset.json"),
