...
**MCSManager 面板**（簡稱：MCSM 面板），一鍵安裝，分散式架構，多用戶出租，現代化的 Minecraft 和 Steam 遊戲伺服器 Web 管理面板。

此軟體在 `Minecraft` 和 `Steam` 遊戲社區內中已有一定的流行程度，它可以幫助你集中管理多個實體伺服器，實現在任何主機上創建遊戲伺服器，並且提供安全可靠的多用戶權限系統，可以很輕鬆的幫助你管理多個伺服器，一直在為 `Minecraft`，`Terraria` 和 ``am` 遊戲伺服器的開發者，提供者支援服務人員的軟體支援。

它同樣也適合任何商業性質的行為，例如 IDC 服務商用於私服出售等，現已有多家中小型企業使用此面板作為銷售軟體。
