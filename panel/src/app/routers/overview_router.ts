...
router.get("/", permission({ level: ROLE.ADMIN, token: false }), async (ctx) => {
  ...
  const requestTasks = Array.from(RemoteServiceSubsystem.services.entries()).map(
    async ([_, remoteService]) => {
      ...
      remoteInfo.publicAddr = remoteService.config.publicAddr;
      remoteInfo.webDirectAddress = remoteService.config.webDirectAddress;
      remoteInfo.webDirectPort = remoteService.config.webDirectPort;
